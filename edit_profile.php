...
if (isset($_POST['edit_profile'])) {
  ...
  $_SESSION['valuesTel'] = $_POST['updateTel'];

  if (!empty($updateName) &&  !empty($updateTel) &&  !empty($updateEmail) ) {
    $loggedInUser = $_SESSION['user_name'];
    $sql = "UPDATE `users` SET `name` = '$updateName', `tel` = '$updateTel', `email` = '$updateEmail' WHERE `name`  = '$loggedInUser'";
    $results = mysqli_query($connect,$sql);
    if ($results) {
      header('Location: profile.php?success=Данные обновлены');
    }
    else {
      header('Location: profile.php?alert=Данные не обновлены');
    }
  }

  else {
    header('Location: profile.php?alert=Пустые поля');
  }
  ...
  if ($query_run) {
    ...
    if ($stmt) {
      ...
      foreach ($_SESSION["shooping-cart"] as $key => $values) {
        ...
        $row['request'] = "Заявка обрабатывается";
        $request = $row['request'];
        $name_order = $name;
